...
import { AccessInterface, predefinedAccess } from '../builtinModels/access';
import ShuttleModel, {
  isShuttleModel,
  shuttleModelConsts,
  supportedInterface,
...
} from '../builtinModels/shuttleModel';
...
export class MongooseBuilder
  implements
    ModelBuilderInterface<ShuttleModelInterface | mongoose.Model<any>, mongoose.Model<any> | null> {
  constructor() {
    this.storedModels = {};
    this.acl = {};
  }
  ...
  build(input: ShuttleModelInterface | mongoose.Model<any>): mongoose.Model<any> | null {
    ...
    try {
      if (isShuttleModel(input)) {
        ...
        Object.entries(input.content).forEach(([key, value]) => {
          coreSchema[key] = MongooseBuilder.parseProperty(value as ShuttleModelProperty);
        });
